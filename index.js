...
╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯`,
            ...
            // 1. AUTO READ STATUS (FAST VIEW)
if (conf.AUTO_READ_STATUS === "yes") {
    try {
        // If ms is an array of status updates
        if (Array.isArray(ms)) {
            await Promise.all(ms.map(async status => {
                try {
                    await zk.readMessages([status.key]);
                    console.log(`Status from ${status.key.remoteJid} read`);
                } catch (err) {
                    console.log("Failed to read status:", err.message);
                }
            }));
        } else {
            // Single status
